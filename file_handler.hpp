#ifndef _bitmap_viewer_file_handler_hpp_INCLUDED_
#define _bitmap_viewer_file_handler_hpp_INCLUDED_
...
std::size_t init< true >::undefined_count(tools::bitmap< T > const& bitmap){
	return std::count_if(bitmap.cbegin(), bitmap.cend(), [](T v){return v != v;});
}
...
public:
	...
	bitmap_info(std::ifstream& is, tools::big::header header):
		...
		undefined_count(init< std::is_floating_point< T >::value >::undefined_count(bitmap))
		{}
...
private:
	static tools::bitmap< T > load(std::ifstream& is, tools::big::header header){
		tools::bitmap< T > bitmap(tools::dimensions< 0, 0 >(header.cols, header.rows));
